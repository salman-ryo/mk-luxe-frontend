        ...
        <div className={`transition-all duration-200 ${isFetching && !isLoading ? 'opacity-60 pointer-events-none' : 'opacity-100'}`}>
          ...
          <div className="grid grid-cols-1 gap-4 md:hidden">
            {filteredCategories.map((category) => (
              <div key={category.id} className="p-4 border border-border/80 rounded-xl bg-card/30 backdrop-blur-sm space-y-3">
                <div className="flex items-start space-x-3">
                  <div className="w-12 h-12 rounded-lg overflow-hidden border border-border flex items-center justify-center bg-muted/40 shrink-0">
                    {category.image_url ? (
                      <img
                      ...
                      <FolderTree className="w-6 h-6 text-muted-foreground/60" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="font-semibold text-sm text-foreground truncate">{category.name}</h3>
                      <span className="text-[10px] font-mono font-semibold text-muted-foreground bg-muted/40 px-1.5 py-0.5 rounded border border-border/40 shrink-0">
                        #{category.sort_order}
                      </span>
                    </div>
                    <p className="text-xs font-mono text-muted-foreground truncate">{category.slug}</p>
                    <p className="text-xs text-muted-foreground/90 line-clamp-2 mt-1">{category.description}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border/50 text-xs">
                  <div className="flex items-center space-x-1.5 flex-wrap">
                    ...
                    {category.is_featured && (
                      <Badge variant="gold" className="text-[10px] space-x-1">
                        <Star className="w-2.5 h-2.5 fill-champagne-gold text-champagne-gold" />
                        <span>Featured</span>
                      </Badge>
                    )}
                  </div>
